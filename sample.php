<?php
     ...
     function searchActicle($page_adress){
          ...
          $result = mysql_query($query) or die("<p>Невозможно получить адрес страницы: " . mysql_error() . "</p>");
     ...
     function searchUser($username){//ищем юзера по url возвращаем в качестве результата всю строку row
          $query="SELECT id, first_name, last_name, network_url FROM users WHERE first_name='{$username['first_name']}'".
     ...
     function addComment($article_id, $user_id, $comment){//добавляем комментарий
          ...
          echo "$user_id ";
          echo "$comment ";
          $query= "INSERT INTO comments (news_id, user_id, comment) VALUES ('{$article_id}', '{$user_id}', '{$comment}');";
     ...
     if(isset($username)){
          ...
          if($user_id){//пишем коммент
            ...
            if(comment){echo "<br> Комментарий добавлен";}
            else{echo "<br> Комментарий НЕ добавлен";}
          }
